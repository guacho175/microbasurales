...
class DenunciaAdminSerializer(DenunciaSerializer):
    ...
    def _crear_notificacion_estado(self, denuncia, nuevo_estado):
        ...
        self._enviar_email_notificacion(denuncia, mensaje)

    def _enviar_email_notificacion(self, denuncia, mensaje):
        """Envía (si está habilitado) un correo al ciudadano. Nunca bloquea ni falla."""

        if not getattr(settings, "NOTIFICACIONES_EMAIL_ENABLED", False):
            return

        correo = getattr(denuncia.usuario, "email", "") or ""
        if not correo.strip():
            return

        try:
            send_mail(
                subject=f"Actualización de tu denuncia #{denuncia.id}",
                message=mensaje,
                from_email=None,  # usa DEFAULT_FROM_EMAIL
                recipient_list=[correo],
                fail_silently=True,
            )
        except Exception:  # noqa: BLE001 - el email nunca debe romper la actualización
            logger.warning(
                "No se pudo enviar el email de notificación de la denuncia #%s.",
                denuncia.id,
                exc_info=True,
            )
...
class DenunciaMapaPublicoSerializer(serializers.ModelSerializer):
    """Vista pública/ciudadana del mapa: solo datos no personales.

    Excluye a propósito al denunciante, la dirección textual, la imagen y la
    descripción para no exponer información personal de quien reportó.
    """
