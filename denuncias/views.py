...
from .permissions import PuedeGestionarDenuncias, PuedeEditarDenunciasFinalizadas
from .serializers import (
    DenunciaAdminSerializer,
    DenunciaCiudadanoSerializer,
    DenunciaMapaPublicoSerializer,
    DenunciaSerializer,
...
class DenunciaListCreateView(APIView):
    """Crea denuncias (cualquier autenticado) y lista todas (solo roles de gestión).

    El listado completo expone denuncias de todos los usuarios, por lo que el GET queda
    restringido a personal de gestión. Los ciudadanos consultan sus denuncias vía
    ``MisDenunciasListView`` (``/api/denuncias/mis/``).
    """
    ...
    def get_permissions(self):
        if self.request.method == "GET":
            return [permissions.IsAuthenticated(), PuedeGestionarDenuncias()]
...
class DenunciaMapaPublicoView(generics.ListAPIView):
    """Mapa público: denuncias sin datos personales del denunciante.

    Acceso anónimo (``AllowAny``): cualquier visitante puede ver el mapa. El serializer
    excluye a propósito la PII del denunciante (usuario, descripción, dirección textual,
    imagen) y solo expone id, ubicación (lat/lon), estado, zona y fecha.
    """

    serializer_class = DenunciaMapaPublicoSerializer
    permission_classes = [permissions.AllowAny]

    def get_queryset(self):
        queryset = Denuncia.objects.only(
            "id", "latitud", "longitud", "estado", "zona", "fecha_creacion"
        )
...
class DenunciaAdminListView(generics.ListAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated, PuedeGestionarDenuncias]
...
class DenunciaAdminUpdateView(generics.UpdateAPIView):
    ...
    # La validación de transiciones de estado (permisos por rol, jefe de cuadrilla
    # obligatorio, motivo de rechazo, transiciones válidas) vive en una sola capa:
    # DenunciaAdminSerializer.validate. No se duplica aquí.
...
def _usuario_puede_gestionar_denuncias(usuario) -> bool:
    ...
    Fuente única: ``Usuario.puede_gestionar_denuncias``.
    """

    return bool(getattr(usuario, "puede_gestionar_denuncias", False))
